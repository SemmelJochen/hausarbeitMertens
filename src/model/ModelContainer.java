...
public class ModelContainer implements Externalizable {
	...
	/*
	 * because of the private constructor it is not possible to have more than one
	 * instance of ModelContainer, which allows us to read the ModelContainer at
	...
	public void updateStudent(Student oldStudent, Student newStudent) {
		...
		this.validateSecondReviewerRequestState(oldStudent, newStudent);
		/**
		 * Students at the peerReviewer are being handled as a whole student. The
		 * problem is, that if we change values at the "original" list of students, the
		 * redundant list at the peer reviewer is not being updated. Therefore we look
		 * for the changed student at the peerReviewer list and replace it, with the
		 * changed one.
		 */
		this.updateStudentReferences(oldStudent, newStudent);

		/**
		 * The same goes in the other direction. If we change the peerReviewer reference
		 * at the student, we also have to check if the new relation is valid. If it is,
		 * we have to delete the reference at the old peerReviewer and add a new
		 * reference to the new selected peerReviewer.
		 */
	 ...
	 * @param newStudent
	 */
	private void validateSecondReviewerRequestState(Student oldStudent, Student newStudent) {
		if (newStudent.getSecondPeerReviewerState().equals(Student.ACCEPTED_STATE)) {
			this.peerReviewers.get(newStudent.getSecondPeerReviewerKey()).acceptRequest(oldStudent);
		}
		if (newStudent.getSecondPeerReviewerState().equals(Student.REJECTED_STATE)) {
			this.peerReviewers.get(newStudent.getSecondPeerReviewerKey()).declineRequest(oldStudent);
			newStudent.setSecondPeerReviewerState(Student.PENDING_STATE);
			newStudent.setSecondPeerReviewerKey("");
		}
	}
	...
	/*
	 * replaces the oldStudent by newStudent in SecondPeerReviewer and replaces the
	 * SecondPeerReviewer in Student as requested
	 */
	...
	/*
	 * replaces the oldStudent by newStudent in firstPeerReviewer and replaces the
	 * firstPeerReviewer in Student
	 */
